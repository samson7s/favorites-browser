...
function loadUsers() {
  try {
    ...
    console.log('用户数据文件不存在或读取失败，使用默认用户');
    users = ['默认用户'];
  }
}
...
let iconPath = getIconPath();
  ...
  try {
    ...
    console.log(`使用的图标路径: ${iconPath || '未找到图标文件'}`);
  } catch (error) {
...
function createNewWindow(url) {
  ...
  try {
    ...
    if (!fs.existsSync(iconPath)) {
      console.log(`资源目录下未找到图标文件: ${iconPath}，尝试使用开发环境路径`);
    ...
    console.log(`新窗口使用的图标路径: ${iconPath}`);
  } catch (error) {
    console.error('获取新窗口图标路径失败:', error);
    iconPath = null;
  }
...
ipcMain.on('get-current-user', (event) => {
  try {
    if (fs.existsSync(favoritesPath)) {
      const data = JSON.parse(fs.readFileSync(favoritesPath, 'utf8'));
      event.reply('current-user-response', data.currentUser || '默认用户');
    } else {
      event.reply('current-user-response', '默认用户');
    }
  } catch (err) {
    ...
    if (success) {
      try {
        ...
        if (fs.existsSync(favoritesPath)) {
          ...
          // 发送更新后的收藏数据（默认用户的数据）
          if (mainWindow && mainWindow.webContents) {
            mainWindow.webContents.send('favorites-updated', data.users['默认用户'] || []);
          }
        }
      } catch (error) {
  ...
  try {
    ...
    if (fs.existsSync(favoritesPath)) {
      ...
      try {
        ...
        const currentUser = data.currentUser || '默认用户';
        // 当前用户
        ...
        try {
          ...
          if (fs.existsSync(backupFavoritesPath)) {
            ...
            const currentUser = backupData.currentUser || '默认用户';
            event.sender.send('favorites-updated', backupData.users?.[currentUser] || []);
            return;
          }
        } catch (backupErr) {
          console.error('使用备用收藏文件也失败:', backupErr.message);
        }
...
ipcMain.on('add-favorite', (event, newItem) => {
  try {
    ...
    dialog.showErrorBox('收藏保存失败', `错误原因：${err.message}`);
  }
});
...
// 保存集数数据
async function saveEpisodeData({ index, total, downloaded }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (
        updatedUserFavorites[index].totalEpisodes !== total ||
...
// 处理添加收藏确认事件
ipcMain.on('add-favorite-confirm', (event, { title, url, openMethod, pinToTop }) => {
  try {
    ...
    dialog.showErrorBox('添加失败', `错误原因：${err.message}`);
  }
});
  ...
  return new Promise((resolve) => {
    ...
    dialogWindow.webContents.on('did-finish-load', () => {
      try {
        ...
        dialogWindow.webContents.send('edit-dialog-error', {
          field: 'general',
          message: `加载收藏数据失败: ${error.message}`
        });
        setTimeout(() => dialogWindow.close(), 3000);
      }
    });
...
ipcMain.handle('show-episode-dialog', async (event, index) => {
  return new Promise((resolve) => {
    ...
    dialogWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
      ...
      dialogWindow.webContents.send('episode-dialog-error', `无法加载对话框: ${errorDescription}`);
    });
    ...
    const handleConfirm = (event, data) => {
      saveEpisodeData(data).then(() => {
        ...
        dialogWindow.webContents.send('episode-dialog-error', `保存失败: ${err.message}`);
      });
      // 移除监听器
    ...
    dialogWindow.webContents.on('did-finish-load', () => {
      try {
        ...
        const item = userFavorites[index];
        if (!item) {
          dialogWindow.webContents.send('episode-dialog-error', `无法找到对应的收藏项 (索引: ${index})，请刷新页面后重试`);
          return;
        }
        ...
        dialogWindow.webContents.send('episode-dialog-error', `加载收藏数据失败: ${error.message}`);
        setTimeout(() => dialogWindow.close(), 3000);
      }
    });
  });
...
ipcMain.on('update-favorites', (event, updatedFavorites) => {
  try {
    ...
    dialog.showErrorBox('更新失败', `无法保存收藏数据: ${err.message}`);
  }
});
// 打开新窗口
...
ipcMain.on('open-external-browser', (event, url) => {
  ...
  shell.openExternal(url).catch(err => {
    ...
    dialog.showErrorBox('打开失败', `无法打开外部浏览器：${err.message}`);
  });
});
  ...
  dialog.showOpenDialog(mainWindow, {
    ...
    if (!result.canceled && result.filePaths.length > 0) {
      ...
      try {
        ...
        // 读取现有收藏数据
        let favoritesData = { users: {}, currentUser: '默认用户' };
        if (fs.existsSync(favoritesPath)) {
          favoritesData = JSON.parse(fs.readFileSync(favoritesPath, 'utf8'));
        }
        ...
        // 确保当前用户存在
        if (!favoritesData.users[favoritesData.currentUser]) {
          favoritesData.users[favoritesData.currentUser] = [];
        }
        
        // 将导入的数据添加到当前用户的收藏中
        if (Array.isArray(importedData)) {
          // 直接导入数组格式
          favoritesData.users[favoritesData.currentUser] = [...favoritesData.users[favoritesData.currentUser], ...importedData];
        } else if (importedData.users && importedData.users[favoritesData.currentUser]) {
          // 导入完整的收藏数据格式
          favoritesData.users[favoritesData.currentUser] = [...favoritesData.users[favoritesData.currentUser], ...importedData.users[favoritesData.currentUser]];
        } else if (importedData.title && importedData.url) {
          // 导入单个收藏项
          favoritesData.users[favoritesData.currentUser].push(importedData);
        }
        
        // 保存更新后的数据
        ensureDirectoryExists(favoritesDir);
        fs.writeFileSync(favoritesPath, JSON.stringify(favoritesData, null, 2), 'utf8');
...
ipcMain.on('export-favorites', (event) => {
  ...
  dialog.showSaveDialog(mainWindow, {
    title: '导出收藏夹',
    defaultPath: `收藏夹备份_${new Date().toISOString().slice(0, 10)}.json`,
    filters: [
      { name: 'JSON Files', extensions: ['json'] }
    ]
  }).then(result => {
    if (!result.canceled && result.filePath) {
      ...
      try {
        ...
        // 导出为JSON文件
        fs.writeFileSync(filePath, JSON.stringify(userFavorites, null, 2), 'utf8');
